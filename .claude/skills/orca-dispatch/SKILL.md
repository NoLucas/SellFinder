...
```bash
orca terminal list --include-visual-layouts --json 2>&1 | python -c "
import json,sys,re
...
```bash
orca terminal send --terminal <handle> --text '<지시문>' --enter --json
```
...
한글·`§`·`—` 가 깨지는 환경이 있다. **한 창에 먼저 보내고 읽어서 확인한 뒤** 나머지에 보내라.

```bash
orca terminal read --terminal <handle> --limit 30 --json 2>/dev/null | python -c "
import json,sys
t=json.loads(sys.stdin.read())['result']['terminal']
print('status:', t['status'])
for l in t['tail'][-12:]: print(' ', l.strip()[:95])
"
```
