...
>
> **6회차 갱신 (2026-08-17 · HEAD `8e047fe`)**: 5절(evidence 작성 규칙 §6) 4개 조항 전부
> `intelligence/tests/test_evidence_rules.py`(`95d8a8e`)로 자동 테스트가 생겼고, 검증자가
> 각 조항마다 별도의 변이를 주입해 실제로 잡히는지 확인했다 — **5절 전체가 O 로 전환됐다.**
> 상세는 `verification/FINDINGS.md` "회차: 6회차"를 봐라.
...
| 계약 조항 | 출처 | 강제하는 테스트 | 실행 확인 | 판정 |
|---|---|---|---|---|
| evidence 가 실제 피처값을 인용 | §6.1 | `test_factor_model.py:75,88` + `test_evidence_rules.py`(`95d8a8e`, 7개) | 통과 | 부분 → (5회차: 위반 없음) → **(6회차: O)** 커밋된 자동 테스트 확인 + 검증자 독립 변이(category_penetration 값 은닉) 주입 시 즉시 실패 |
| evidence 에 비교 기준 동반 | §6.2 | `test_evidence_rules.py`(`95d8a8e`) | 통과 | **구멍** → (5회차: 위반 없음, 테스트 아직 없음) → **(6회차: O)** 커밋됨 + 검증자 독립 변이(competition 비교 표현 제거) 주입 시 즉시 실패 |
| 모델이 안 쓴 근거를 지어내지 않음 (null 피처 인용 금지) | §6.2 | `test_evidence_rules.py`(`95d8a8e`) | 통과 | **구멍** → (5회차: 위반 없음) → **(6회차: O)** 커밋됨 + 검증자 독립 변이(price_acceptance 가 null income_decile 로 소득분위 지어냄) 주입 시 즉시 실패 — CHARTER §5.1 3번째 항목, 이번 회차에 닫힘 |
| 인과 주장 금지 | §6.3 | `test_evidence_rules.py`(`95d8a8e`) | 통과 | **구멍** → (5회차: 위반 없음) → **(6회차: O)** 커밋됨 + 검증자 독립 변이(addressable_demand 에 인과 문구 삽입) 주입 시 즉시 실패 |
