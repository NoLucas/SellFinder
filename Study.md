...
이음매 테스트를 만들어도 **자동으로 돌지 않는다.** 사람이 기억해서 돌려야 한다.

**개선**: GitHub Actions 로 `pytest` + `node --test` + `validate_contracts.py` + 폴더 경계 검사를
PR 마다 자동 실행. 4강의 사고는 CI 가 있었으면 훨씬 빨리 잡혔다.

### 11.4 evidence 규칙이 강제되지 않는다
...
**네 조항 전부 아무 테스트도 강제하지 않는다.** 지금까지는 진짜 예측이 없어 검증조차 불가능했다.
...
### 11.6 저장소에 가상환경이 커밋돼 있었다

`data-platform/.venv` 파일 2,902개가 추적되고 있었다. `.gitignore` 에 규칙은 **있었지만**
규칙보다 먼저 커밋돼서 소용이 없었다.

> [!info] gitignore 는 이미 추적 중인 파일에 효력이 없다
> 해결: `git rm -r --cached <경로>` 로 추적만 해제한다 (파일은 안 지워진다).
> 단 **히스토리에는 남으므로 clone 크기는 줄지 않는다.**

---

## 12강. 정리 — 이 프로젝트가 가르쳐주는 것
