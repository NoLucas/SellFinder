...
ADR 은 `00`~`06` 을 **덮어쓴다.** 번호가 뒤에 붙었다고 부록이 아니다.
본문과 ADR 이 다르면 **ADR 이 이긴다** (본문에는 취소선과 대체 번호를 남긴다).

| # | 파일 | 내용 | 필독 대상 |
|---|------|------|-----------|
| 001 | `ADR-001-map-tiles.md` | 경계 타일(A)·점수(C) 분리, 클라이언트 조인 | A, C, D |
| 002 | `ADR-002-artifact-publishing.md` | 아티팩트 발행 경로 · 지역 레벨 · 줌 범위 | A, C, D |
| 003 | `ADR-003-auth.md` | JWT 클레임 · `verify_token` 단일 지점 · 테넌트 강제 | C, D |
| 004 | `ADR-004-taxonomy-mapping.md` | `sbiz` 1차 조인 키, `card_mcc` 보류 | A, B |
| 005 | `ADR-005-tile-join-key.md` | **조인 키는 properties 의 `region_id` 문자열** | A, C, D |

> ADR-005 는 ADR-001 안에서 A 와 D 가 서로 반대 지시를 받고 있던 것을 정정한다.
> 두 에이전트 모두 자기 문서를 지켰는데 지도가 통째로 비었다 (검증 1회차 VF-003).
> **계약 문서끼리 모순되면 그것은 에이전트가 아니라 계약의 결함이다. 발견하면 CCR 로 올려라.**
